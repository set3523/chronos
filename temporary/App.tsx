...
function App() {
  ...
  const getLabel = (key: string): string => {
    const labels: {[key: string]: string} = {
      [AspectRatio.Portrait]: "Î™®Î∞î??(9:16)",
      [AspectRatio.Landscape]: "?∞Ïä§?¨ÌÉë (16:9)",
      [AspectRatio.Square]: "?ïÎ∞©??(1:1)",
      [Environment.Forest]: "ÍπäÏ? ?≤ÏÜç",
      [Environment.Beach]: "Î∞§Î∞î??,
      [Environment.Snowy]: "?§Ïõê",
      [Environment.SimpleBlack]: "?¨Ìîå Î∏îÎûô",
      [FuelType.Wood]: "?•Ïûë (?òÎ¨¥)",
      [FuelType.Paper]: "Ï¢ÖÏù¥/ÏßÄ?∏ÎùºÍ∏?,
      [FuelType.Charcoal]: "??(Charcoal)",
      [FuelType.Chemical]: "?îÌïô Î¨ºÏßà",
      [Weather.Clear]: "ÎßëÏùå",
      [Weather.Rainy]: "Îπ?,
      [Weather.Snowy]: "??,
      [Weather.Windy]: "Î∞îÎûå",
    };
      ...
      <div className="w-full md:w-[450px] flex-shrink-0 flex flex-col border-r border-white/10 glass-panel h-screen overflow-y-auto z-10 relative">
        <div className="p-6 border-b border-white/10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Î¨ºÎ¶¨ Í∏∞Î∞ò ?åÌã∞???îÏßÑ</p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-8 flex-1">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <div className="flex items-center justify-between mb-4">
                  <span className="text-sm font-medium text-gray-200">?∞ÏÜå ?Ä?¥Î®∏</span>
                  <button 
                  ...
                  >
                      {isExtendedTimer ? "Í∞ÑÌé∏ Î™®Îìú (??Î∂?Ï¥?" : "?ïÏû• Î™®Îìú (??????..)"}
                  </button>
              </div>
              ...
              <div className="flex gap-2">
                  ...
                  >
                      {isTimerRunning ? "?Ä?¥Î®∏ ?¨ÏÑ§??(Restart)" : "Î∂?Î∂ôÏù¥Í∏?(Ignite)"}
                  </button>
                  {isTimerRunning && (
                    <button 
              ...
              <p className="text-[10px] text-gray-500 mt-2 text-center">
                  ?§Ï†ï???úÍ∞Ñ ?ôÏïà Î∂àÏù¥ ?Ä?§Î•¥Î©? ?∞Î£åÍ∞Ä ???òÎ©¥ ?úÏÑú??Í∫ºÏßë?àÎã§.
              </p>
          </div>

          {/* Aspect Ratio */}
          <div className="space-y-3">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">?îÎ©¥ ?§Ï†ï</label>
              <div className="grid grid-cols-3 gap-2">
          ...
          {/* Environment & Weather */}
          <div className="space-y-3">
             <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">?òÍ≤Ω ?§Ï†ï</label>
             <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="text-xs text-gray-400 block mb-2">Î∞∞Í≤Ω</label>
                    <select
                ...
                <div>
                    <label className="text-xs text-gray-400 block mb-2">?†Ïî®</label>
                    <select
          ...
          {/* Fuel Type */}
           <div className="space-y-3">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">?∞Î£å (?¨Ïßà)</label>
